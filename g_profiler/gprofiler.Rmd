...
```{r}
E2F.gostres.all <- gost(query = E2F$HALLMARK_E2F_TARGETS,organism = "hsapiens", 
                    sources = c("GO:BP", "GO:MF", "GO:CC", "KEGG", "REAC", "WP"),
                    significant = FALSE,  correction_method ="g_SCS",
                    domain_scope="annotated")
...
g1vrev.Revdegs.gostres.bg <- gost(query = g1vrev.Revdegs$Gene,organism = "hsapiens", 
                sources = c("GO:BP", "GO:MF", "GO:CC", "KEGG", "REAC", "WP"),
                significant = FALSE, correction_method ="g_SCS",
                custom_bg = g1vrev.bkground$Gene, domain_scope="custom")

write.xlsx(g1vrev.Revdegs.gostres.bg$result %>% select(c("term_id", "term_name","source","p_value", "query_size", "intersection_size")), file="g1vrev.RevDegs.gprofiler.xlsx")
```
